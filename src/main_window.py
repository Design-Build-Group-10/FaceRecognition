...
class MainWindow(QMainWindow):
    ...
    def upload_image(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(self, "选择图片文件", "", "Images (*.png *.jpg *.bmp);;All Files (*)", options=options)
        if file_name:
            self.image = cv2.imread(file_name)
            self.display_image(self.left_image_label, self.image, self.left_image_label.width(), self.left_image_label.height())
            ...
            self.display_image(self.middle_image_label, processed_frame, self.middle_image_label.width(), self.middle_image_label.height())
            self.display_image(self.right_image_label, whiteboard, self.right_image_label.width(), self.right_image_label.height())
            self.update_images("entry", processed_frame, whiteboard, unknown_faces, unknown_embeddings)

    def upload_video(self):
        # 打开文件选择对话框，让用户选择视频文件
        video_file, _ = QFileDialog.getOpenFileName(self.video_recognition_tab, "选择视频文件", "", "视频文件 (*.mp4 *.avi *.mov)")
        # 检查用户是否选择了文件
        if video_file:
            # 显示处理中的状态
    ...
    def update_images(self, tab, processed_frame, whiteboard, unknown_faces, unknown_embeddings):
        ...
        if tab == "entry":
            ...
            self.display_image(self.processed_label, processed_frame, self.processed_label.width(), self.processed_label.height())
            self.display_image(self.whiteboard_label, whiteboard, self.whiteboard_label.width(), self.whiteboard_label.height())
    ...
    def process_unknown_faces(self, tab, containers, unknown_faces, unknown_embeddings):
        ...
        for face_image, embedding in zip(unknown_faces, unknown_embeddings):
            ...
            confirm_button.clicked.connect(lambda _, user_id=id_input, embedding=embedding: save_to_chroma(user_id.text(), embedding, containers, unknown_embeddings))
